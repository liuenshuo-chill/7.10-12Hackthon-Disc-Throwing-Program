...
============================================================================
  1. 下面的 import 假设 4_red_disc_detection.py / red_teammate_detection.py /
     ...
     动态加载"的兼容处理，两种情况都能跑。
  2. GRASP_TRIGGER_DEPTH_M（阶段1判断"够近可以夹"的深度阈值）、
     SCAN_JOINT1_SPEED（阶段2环视角速度）都需要你在真机上实测调一版。
...
# 现在只用在阶段2扫描开始前（见 scan_for_teammate 里的 HOME_J1_MARGIN 用法），
# 不再影响阶段0/阶段1。
HOME_JOINT_POS = [0.0, 0.785, 0.785, 0.0, 0.0, 0.0]
HOME_J1_MARGIN = 0.10   # rad，阶段2开始前，离关节1最负端限位留的安全余量，TODO按真机实测调整
...
# TODO: 相机装在末端(眼在手上)，z3d 就是相机到飞盘的距离，
#       在真机上实测夹爪合适下手时机对应的 z3d 再填这里
GRASP_TRIGGER_DEPTH_M = 0.08
GRASP_STABLE_FRAMES   = 15     # 连续多少帧 z3d 都小于阈值才真正触发抓取，防抖动误触发
...
def approach_and_grasp(robot, pipeline, align, intrinsic, depth_scale,
    ...
    print("\n[阶段1] 开始视觉伺服接近红色飞盘...（按 q 退出程序，按 m 切换掩膜）")
    ...
    try:
        while True:
            ...
            # 相机一旦抖动/掉线就会变成真正意义上按'q'都退不出的死循环。现在把按键检测
            # 挪到取帧校验之前，保证任何时候都能响应'q'/'m'。
            ...
            if time.time() - stage_start_time > STAGE1_NO_GRASP_TIMEOUT_S:
                raise RuntimeError(
                    f"阶段1: 超过 {STAGE1_NO_GRASP_TIMEOUT_S:.0f}s 仍未完成抓取，"
                    f"请检查飞盘是否在视野内、GRASP_TRIGGER_DEPTH_M 是否合理"
                    f"（当前{GRASP_TRIGGER_DEPTH_M}m，注意别太靠近相机 MIN_DEPTH_MM="
                    f"{MIN_DEPTH_MM}mm 的量程盲区）"
                )
            ...
            cv2.putText(display, "Stage1: approach & grasp frisbee",
            ...
            # ── 抓取触发判断：相机(装在末端)到飞盘的距离连续多帧小于阈值 ──
            if nearest['z3d'] < GRASP_TRIGGER_DEPTH_M:
                stable_count += 1
            else:
                stable_count = 0

            if time.time() - last_status_print > STATUS_PRINT_INTERVAL_S:
                print(f"[阶段1] 状态: 跟踪飞盘中，z3d={nearest['z3d']:.3f}m "
                      f"(触发阈值{GRASP_TRIGGER_DEPTH_M:.3f}m)，"
                      f"确认帧 {stable_count}/{GRASP_STABLE_FRAMES}")
                last_status_print = time.time()

            if stable_count >= GRASP_STABLE_FRAMES:
                print(f"[阶段1] 飞盘已进入抓取范围 (z3d={nearest['z3d']:.3f}m)，闭合夹爪")
...
def scan_for_teammate(robot, pipeline, align, yolo_model):
    ...
    lower0 = robot.joint_limits['lower'][0] if robot.joint_limits is not None else None
    upper0 = robot.joint_limits['upper'][0] if robot.joint_limits is not None else None

    # ── 先转到扫描起始位置：关节1单独转到"最负端限位 + 安全余量"，其余关节 ──
    # 保持阶段1抓取结束时的姿态不变（不想因为挪动其余关节而松动抓着的飞盘）。
    # 这一步同样是直接下发关节空间目标角度，不经过FK/IK；速度用 HOME_JOINT_VEL，
    # 不求快只求到位。
    post_grasp_pos = list(robot.get_current_pos())
    other_pos = post_grasp_pos[1:]     # 关节2~6在扫描过程中全程保持不动

    if robot.joint_limits is not None:
        scan_start_j1 = float(robot.joint_limits['lower'][0]) + HOME_J1_MARGIN
    else:
        scan_start_j1 = post_grasp_pos[0]
        print("[阶段2] 警告: 未加载关节限位，扫描起始角度退化为当前角度，请自行确认安全")

    scan_start_pos = [scan_start_j1] + other_pos
    print(f"[阶段2] 先转到扫描起始位置(关节1={scan_start_j1:.3f}rad，最负端+余量): "
          f"{np.round(scan_start_pos, 3)}")
    ok = robot.Joint_Pos_Vel(scan_start_pos, HOME_JOINT_VEL, None, iswait=True)
    if not ok:
        raise RuntimeError("阶段2: 移动到扫描起始位置被限位保护拒绝，请检查 HOME_J1_MARGIN 设置")
...
def rotate_90_with_limit_check(robot, current_pos):
    ...
    if robot.joint_limits is not None:
        ...
        if abs(clipped_j1 - desired_j1) > 1e-9:
            print(f"[阶段3] 警告: 正转90°会超出关节1限位 [{lower0:.3f}, {upper0:.3f}] rad，"
